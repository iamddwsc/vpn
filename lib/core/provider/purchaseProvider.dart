...
class PurchaseProvider extends ChangeNotifier {
  ...
  Future<void> _verifyPurchase(
      BuildContext context, PurchasedItem detail) async {
    ...
    switch (detail.productId!.toLowerCase()) {
      ...
      case 'three_days_subs':
        purchaseTime = purchaseTime!.add(const Duration(days: 3));
        break;
      default:
        return;
    }
    if (DateTime.now().isBefore(purchaseTime)) {
      vpnProvider.proLimitDate = purchaseTime;
    }
    try {
  ...
  static PurchaseProvider instance(BuildContext context) =>
